...
import com.sdg.cmdb.dao.AuthDao;
...
public class AuthServiceImpl implements AuthService, InitializingBean {
    ...
    public UserDO getUserByName(String username) {
        ...
        try {
            ...
            DirContextAdapter adapter = (DirContextAdapter) ldapFactory.getLdapTemplateInstanceByType(LdapDO.LdapTypeEnum.cmdb.getDesc()).lookup(dn);
    ...
    public BusinessWrapper<Boolean> delUser(String username) {
        ...
        try {
            ...
            if (StringUtils.isEmpty(userDO.getMail())) {
                String accountStatus = getMailAccountStatus(username + "@51xianqu.net");
                if (!accountStatus.equalsIgnoreCase("null"))
                    userLeaveDO.setMail(username + "@51xianqu.net");
    ...
    public List<String> searchBambooGroupFilter(String username) {
        ...
        try {
            groups = ldapFactory.getLdapTemplateInstanceByType(LdapDO.LdapTypeEnum.cmdb.getDesc()).search(LdapQueryBuilder.query().base(groupDN)
    ...
    public boolean isGroupMember(String username, String groupName) {
        ...
        String groupDN = configMap.get(LdapItemEnum.LDAP_GROUP_DN.getItemKey());
        String dn = "cn=" + username + "," + userDN;
        List<String> groups = ldapFactory.getLdapTemplateInstanceByType(LdapDO.LdapTypeEnum.cmdb.getDesc()).search(LdapQueryBuilder.query().base(groupDN)
    ...
    private boolean chgMemberToGroup(UserDO userDO, ServerGroupDO serverGroupDO, int type) {
        ...
        try {
            ldapFactory.getLdapTemplateInstanceByType(LdapDO.LdapTypeEnum.cmdb.getDesc()).modifyAttributes(dn, new ModificationItem[]{
...
//            if (org.springframework.util.StringUtils.isEmpty(userDO.getMobile()))
//                attrs.put("telephoneNumber", userDO.getMobile());
                //attrs.put("title", StringUtils.trimToEmpty(vo.getTitle()));
                attrs.put("userpassword", userDO.getPwd());
                ldapFactory.getLdapTemplateInstanceByType(LdapDO.LdapTypeEnum.cmdb.getDesc()).bind(dn, null, attrs);
                // 模拟登录插入本地用户数据
    ...
    public boolean setMailUserAccountStatus(UserDO userDO, String status) {
        ...
        if (StringUtils.isEmpty(userDO.getMail())) {
            ...
            dn = "uid=" + userDO.getMail().replaceAll("@51xianqu.net", "") + "," + userDN;
        }

        try {
            ldapFactory.getLdapTemplateInstanceByType(LdapDO.LdapTypeEnum.mail.getDesc()).modifyAttributes(dn, new ModificationItem[]
                    {
    ...
    public String getMailAccountStatus(String mail) {
        ...
        String username = mail.replaceAll("@51xianqu.net", "");
        String dn = "uid=" + username + "," + userDN;
        try {
            DirContextAdapter adapter = (DirContextAdapter) ldapFactory.getLdapTemplateInstanceByType(LdapDO.LdapTypeEnum.mail.getDesc()).lookup(dn);
