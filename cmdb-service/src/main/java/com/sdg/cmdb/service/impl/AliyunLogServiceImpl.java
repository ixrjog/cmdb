...
import com.sdg.cmdb.dao.LogServiceDao;
...
//    }
    ...
    public LogServiceVO queryLog(LogServiceQuery logServiceQuery) throws LogException {
        ...
        try {
            String date = logServiceQuery.acqQueryDate();
            long queryFrom = TimeUtils.dateToStamp(date);
            int from = (int) (queryFrom / 1000);
            int to = from + logServiceQuery.acqToMinutes() * 60;
...
//            System.err.println("logstore:"+ logstore);
...
//        LogServicePathDO logServicePathDO = logServiceDao.queryLogServicePath(tagPath, serverGroupId);
//
//        if (logServicePathDO != null) {
//            logServicePathDO.setSearchCnt(logServicePathDO.getSearchCnt() + 1);
//            logServiceDao.updateLogServicePath(logServicePathDO);
//        } else {
//            logServicePathDO = new LogServicePathDO(tagPath, serverGroupId);
//            logServiceDao.addLogServicePath(logServicePathDO);
//        }
    }
    ...
    private String acqQuery(LogServiceKaQuery kaQuery) {
        ...
        if (!StringUtils.isEmpty(kaQuery.getRequestTime())) {
            query += (StringUtils.isEmpty(query)) ? "" : "";
