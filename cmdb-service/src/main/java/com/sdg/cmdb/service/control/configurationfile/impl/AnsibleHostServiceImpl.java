...
public class AnsibleHostServiceImpl implements AnsibleHostService {
     ...
     */
    @Override
    public Map<String, List<ServerDO>> groupingWebService(ServerGroupDO serverGroupDO) {
     ...
     */
    @Override
    public Map<String, List<ServerDO>> grouping(ServerGroupDO serverGroupDO) {
    ...
    public List<ServerDO> queryServerGroupByCiGroupName(ServerGroupDO serverGroupDO, String ciGroupName) {
        Map<String, List<ServerDO>> serverMap = groupingWebService(serverGroupDO);
    ...
    public String acqConfig(int type) {
        if (type == 0) return acqHostsCfg();
        if (type == 2) return acqHostsAllCfg();
        return new String();
    }

    @Override
    public String acqHostsCfg() {
        List<ServerGroupDO> serverGroups = serverGroupDao.queryServerGroup();
        ...
        for (ServerGroupDO serverGroup : serverGroups) {
            ...
            if (hosts.isEmpty())
                continue;
            Map<String, List<ServerDO>> serverMap = groupingWebService(serverGroup);
