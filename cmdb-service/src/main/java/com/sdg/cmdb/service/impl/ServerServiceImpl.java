...
public class ServerServiceImpl implements ServerService {
    ...
    @Override
    public TableVO<List<ServerVO>> getZabbixServerPage(long serverGroupId, String serverName, int useType, int envType, String queryIp, int zabbixStatus, int zabbixMonitor, int page, int length) {
        List<Long> groupFilter = authService.getUserGroupIds(SessionUtils.getUsername());
        long size = serverDao.getZabbixServerSize(groupFilter, serverGroupId, serverName, useType, envType, queryIp, zabbixStatus, zabbixMonitor);
        List<ServerDO> list = serverDao.getZabbixServerPage(groupFilter, serverGroupId, serverName, useType, envType, queryIp, zabbixStatus, zabbixMonitor, page * length, length);
        List<ServerVO> voList = new ArrayList<>();
        for (ServerDO serverDO : list) {
            ServerGroupDO serverGroupDO = serverGroupService.queryServerGroupById(serverDO.getServerGroupId());
            IPDetailVO publicIP = ipService.getIPDetail(new IPDetailDO(serverDO.getPublicIpId()));
